...
export default function MarkdownContent({ content, html }: MarkdownContentProps) {
      ...
      if (href && (href.endsWith('.md') || href.startsWith('./') || href.startsWith('../'))) {
        link.addEventListener('click', (e) => {
          ...
          if (newPath.startsWith('../')) {
            ...
            const finalPath = `/learning/${pathParts.join('/')}/${newPath}`.replace(/\/+/g, '/');
    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
      <div className="prose prose-lg max-w-none">
        <ReactMarkdown
          ...
          components={{
          h1: ({node, ...props}) => <h1 className="text-3xl font-bold mb-4 pb-2 border-b border-gray-200" {...props} />,
          ...
          code: ({inline, className, children, ...props}: {inline?: boolean; className?: string; children?: React.ReactNode}) => 
            inline ? 
              <code className={`bg-gray-100 px-1 py-0.5 rounded text-red-600 font-mono text-sm ${className || ''}`} {...props}>{children}</code> : 
              <pre className={`block bg-gray-100 p-4 rounded-md font-mono text-sm overflow-x-auto my-4 ${className || ''}`} {...props}><code>{children}</code></pre>
        }}
      >
          {content}
        </ReactMarkdown>
